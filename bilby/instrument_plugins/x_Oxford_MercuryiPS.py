# Oxford_MercuryiPS.py class, to perform the communication between the Wrapper and the device
...
class Oxford_MercuryiPS(Instrument):
    '''
    This is the python driver for the Oxford Instruments Mercury iPS Magnet Power Supply

    Usage:
    Initialize with
    <name> = instruments.create('name', 'Oxford_MercuryiPS', address='<Instrument address>')
...
#TODO: write in SCPI commands
    ...
    def _execute(self, message):
        ...
        result = '666 i am not real'#self._visainstrument.read()
        if result.find('?') >= 0:
