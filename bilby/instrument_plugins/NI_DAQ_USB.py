...
import types
from lib.dll_support import nidaq_USB
...
#                type=types.FloatType,    #Jan en Max changed 'type=types.FloatType' to 'type=types.ListType, if charge sensing is needed'
    ...
    def reset(self):
        '''Reset device.'''
        nidaq_USB.reset_device(self._id)

    def _get_input_channels(self):
        return nidaq_USB.get_physical_input_channels(self._id)

    def _get_output_channels(self):
        return nidaq_USB.get_physical_output_channels(self._id)

    def _get_counter_channels(self):
        return nidaq_USB.get_physical_counter_channels(self._id)

    def do_get_input(self, channel, average=True, minvol = -10.0, maxvol = 10.0):
        # Gabriele added the average parameter
        devchan = '%s/%s' % (self._id, channel)
        #Jan added samples=self._samples and freq=self._fraq
        values = nidaq_USB.read(devchan, config=self._chan_config, samples=self._samples, freq=self._freq, averaging=average, minv=minvol, maxv=maxvol)
    ...
    def do_set_output(self, val, channel):
        ...
        return nidaq_USB.write(devchan, val/1000.0)#/100 for 1:10 divider, /1000 for normal
    ...
    def do_get_counter(self, channel):
        ...
        return nidaq_USB.read_counter(devchan, src=src, freq=1/self._count_time)
...
def detect_instruments():
    ...
    for name in nidaq_USB.get_device_names():
