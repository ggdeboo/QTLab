...
#
# This interface should be run with "execfile('aff_stabgui.py')"
#
...
class Stabgui:
	def __init__(self):
		...
		self.xml = gtk.glade.XML('aff_stabgui.glade', 'window')
	...
	def initialize_gui(self):
		...
		self.dacstore.append(['Bfield'])
		self.dacstore.append(['tBlock'])
		self.dacstore.append(['tRepeat'])
		self.dacstore.append(['tDelay'])
		self.dacstore.append(['tPulse'])
	...
	def fill_entries(self):
		...
		self.xml.get_widget('pause1').set_active(self.settings['pause1'])
		self.xml.get_widget('pause2').set_active(self.settings['pause2'])
		self.xml.get_widget('addr1').set_active(self.settings['addr1'])
		self.xml.get_widget('addr2').set_active(self.settings['addr2'])
	...
	def get_entries(self):
		...
		self.settings['pause1'] = self.xml.get_widget('pause1').get_text()
		self.settings['pause2'] = self.xml.get_widget('pause2').get_text()
		self.settings['addr1'] = self.xml.get_widget('addr2').get_text()
		self.settings['addr2'] = self.xml.get_widget('addr2').get_text()
...
#-------------------------------------------------------------------------------------------------
	def on_measure_clicked(self,widget):
		...
		MWgen = qt.instruments.create('MWgen','Agilent_E8257C',address=self.settings['addr1'])
		HPPulse = qt.instruments.create('HPPulse','HP_8131A',address=self.settings['addr2'])
		OXMag = qt.instruments.create('OXMag','OxfordInstruments_IPS120',address='COM3')
		...
		pauseT1 = self.settings['pause1']
		pauseT2 = self.settings['pause2']
		# create dacs, first two will be swept 
		v_dac = []
		v_vec = []
		...
		if number_of_dynamic_dacs == 2:
			for v1 in v_vec[0]:
				execute_set(v_dac[0],v1])
				#ivvi.set(v_dac[0],v1)
				qt.msleep(pauseT1)
				for iv2,v2 in enumerate(v_vec[1]):
					execute_set(v_dac[1],v2])
					#ivvi.set(v_dac[1],v2)
					# Pause to let the current meter settle
					qt.msleep(pauseT2)
					# readout
					result = NIDev1.get('ai0')*(1000/I_gain) #1000 is for mV to V
