#  -*- coding: utf-8 -*-
# Agilent_34970A.py driver for Agilent 34970A Data Acquisition Unit
...
class Agilent_34970A(Instrument):
    '''
    This is the driver for the Agilent 34970A Data acquisition unit
    #still needs a bit of debugging

    Usage:
    Initialize with
    <name> = instruments.create('<name>', 'Agilent_34970',
    ...
    def __init__(self, name, address, reset=False):
        '''
        Initializes the Agilent_34970A, and communicates with the wrapper.
        ...
        logging.info('Initializing instrument HP_34970A')
        ...
        # Figure out which modules are installed
        
        self.dmm_installed = bool(int(
            self._visainstrument.query('INST:DMM:INST?')))
        if self.dmm_installed:
            logging.info('DMM is installed')
        else:
            logging.info('DMM is NOT installed')
        self.modules = [''] * 3
        self.modules[0] = self._visainstrument.query('SYST:CTYPE? 100')
        self.modules[1] = self._visainstrument.query('SYST:CTYPE? 200')
        self.modules[2] = self._visainstrument.query('SYST:CTYPE? 300')

        self.chan_list = []

        for idx, module in enumerate(self.modules):
            if '34908A' in module:
                self.chan_list= self.chan_list + (range(100*(idx+1)+1,100*(idx+1)+41))
...
#        for chan in self.chan_list:
        self.add_parameter('function',
            ...
            tags=['measure'],
            get_func=self.do_get_channel_function,
        ...
        self.add_parameter('range',
            ...
            tags=['measure'],
            get_func=self.do_get_channel_range,
            channels=self.chan_list,
            channel_prefix='ch%s_',
            format='%.3e')
        ...
        self.add_parameter('scan_list',
            flags=Instrument.FLAG_GET,
            type=types.ListType,
            )
        self.add_parameter('scan_read',
            flags=Instrument.FLAG_GET,
            type=numpy.ndarray,
            )
...
# --------------------------------------
    ...
    def get_all(self):
        ...
        for chan in self.chan_list:
            self.get('ch{0}_function'.format(str(chan)))
            self.get('ch{0}_range'.format(str(chan)))
            self.get('ch{0}_autorange'.format(str(chan)))
...
#        self.get_last_error_message()
    ...
    def do_get_channel_function(self, channel):
        '''
        Get the function for the channel of a 34908A module
        '''
        conf = self._visainstrument.query(
            'CONF? (@{0})'.format(str(channel)))
        if 'VOLT' in conf:
            return('VOLT')
        elif 'TEMP' in conf:
            return('TEMP')
    ...
    def do_get_channel_range(self, channel):
        '''
        Get the range for the channel of a 34908A module
        '''
        conf = self._visainstrument.query(
            'CONF? (@{0})'.format(str(channel)))
        return float(conf[5:19])

    def do_get_autorange(self, channel):
        '''
        Get whether a channel is set to auto range
        '''
        function = self.get('ch%s_function' % channel)
        return bool(int(self._visainstrument.query(
                'SENS:{0}:DC:RANG:AUTO? (@{1})'.format(
                                                function,
                                                channel)
                                                    ))
                )

    def do_get_scan_list(self):
        '''
        Get the list of the channels that are scanned
        '''
        scan = self._visainstrument.query('ROUT:SCAN?')
        message_length_bytes = int(scan[1])
        message_length = int(scan[2:2+message_length_bytes])
        scan_string = scan[4+message_length_bytes:-2]
        scan_list = scan_string.rstrip(')').split(',')
        scan_list = [int(channel) for channel in scan_list]
        return scan_list

    def do_get_scan_read(self):
        '''
        Do a scan and get the result
        '''
        scan_read = self._visainstrument.query('READ?')
        scan_result = scan_read.split(',')
        scan_result = [float(chan_result) for chan_result in scan_result]
        return numpy.array(scan_result)
